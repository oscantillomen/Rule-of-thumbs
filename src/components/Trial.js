...
const Trial = ({
  data: { id, name, time, category, description, image, votes },
  persons,
  setPersons
}) => {
  ...
  return (
    <article className="trial" style={style} role="region" aria-label="Trial">
      <div className="trial--infoContainer" role="img" aria-label="Image">
        <div className="trial--infoContainer--name">
          <span
            className={`trial--infoContainer--name-thumb ${
              upPercent >= downPercent || (votes.up === 0 && votes.down === 0)
                ? "thumbsUpColor"
                : "thumbsDownColor"
            }`}
          >
          ...
          {voted ? (
            ...
            <>
              <button
                ...
                className="voteUp up"
                onClick={selectThumb}
                aria-label={`Vote thumbs up for ${name}`}
              >
                <FontAwesomeIcon icon="thumbs-up" />
              </button>
              <button
        ...
        <div className="trial--infoContainer--score">
          {downPercent !== 100 && (
            <div
              className="trial--infoContainer--score-up"
              style={{
                width: !upPercent && !downPercent ? "50%" : `${upPercent}%`
              }}
            >
              <FontAwesomeIcon icon="thumbs-up" />
          ...
          {upPercent !== 100 && (
            <div
              className="trial--infoContainer--score-down"
              style={{
                width: !upPercent && !downPercent ? "50%" : `${downPercent}%`
              }}
            >
              <FontAwesomeIcon icon="thumbs-down" />
